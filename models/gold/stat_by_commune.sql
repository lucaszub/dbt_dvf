...
SELECT
    ...
    BA.LATITUDE,
    BA.LONGITUDE
FROM {{ ref('fact_mutation')}} as DVF
LEFT JOIN {{ ref('dim_address')}} as BA
    ON DVF.join_key = BA.join_key
LEFT JOIN {{ ref('dim_type_local')}} as TL
    ON DVF.type_local_sk = TL.type_local_sk
)
select 
    YEAR(DATE_MUTATION) AS ANNEE,    
    CODE_DEPARTEMENT,
    COUNT(*) as nb_vente,
    MEDIAN(VALEUR_FONCIERE) as median_valeur_fonciere
